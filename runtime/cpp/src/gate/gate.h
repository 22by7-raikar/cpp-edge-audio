...
// Defaults are conservative: prefer false-negatives over false-positives.
// -------------------------------------------------------
struct GateConfig {
    // --- Time-domain thresholds ---
    double rms_min              = 0.003;   // below this -> BORDERLINE
    double rms_borderline_min   = 0.001;   // below this -> FAIL
    double max_silence_ratio    = 0.90;    // fraction of silent samples -> FAIL
    double max_clipping_ratio   = 0.05;    // fraction of clipped samples -> FAIL
    ...
    // --- Spectral thresholds (M2) ---
    // Spectral flatness near 1.0 indicates white noise / non-speech.
    // Typically: clean speech ~0.1-0.4, stationary noise ~0.7-1.0.
    double spectral_flatness_max  = 0.90;  // mean flatness above this -> FAIL
    double spectral_flatness_warn = 0.75;  // mean flatness above this -> BORDERLINE
